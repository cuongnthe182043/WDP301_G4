...
export default function Footer() {
  ...
  return (
    <footer style={{ background: "linear-gradient(180deg, #1E3A8A 0%, #1E40AF 40%, #1D4ED8 100%)" }}>
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-blue-300 text-[11px] mt-0.5">{sub}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4">
          ...
          <div className="flex items-center gap-2.5 mb-4">
            ...
            >
              {/* Fallback logo placeholder — replace with actual img if needed */}
              <div className="w-full h-full bg-white flex items-center justify-center">
                <span className="text-blue-700 font-black text-sm">DF</span>
              </div>
            </div>
            <div>
              <p className="text-white font-black text-[18px] tracking-tight leading-tight">Daily Fit</p>
              <p className="text-blue-300 text-[10px] font-semibold uppercase tracking-[0.15em]">Smart Fashion</p>
            </div>
          </div>

          <p className="text-blue-200 text-sm leading-relaxed mb-6 max-w-[280px]">
              ...
              <form onSubmit={handleSubscribe} className="flex gap-2 max-w-[280px]">
                ...
                >
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent border-none outline-none text-sm text-white placeholder-blue-300 min-w-0"
                    required
                  />
                </div>
                <motion.button
          ...
          <ul className="space-y-2.5">
            {[
              ...
              <li key={to}>
                <RouterLink
                  to={to}
                  className="group flex items-center gap-1.5 text-sm text-blue-200 hover:text-white transition-colors no-underline"
                >
          ...
          <ul className="space-y-4 mb-6">
            {[
              ...
              <li key={i} className="flex items-start gap-3">
                ...
                <span className="text-sm text-blue-100 leading-tight pt-1.5">{text}</span>
              </li>
            ))}
          </ul>
          ...
          <div className="flex items-center gap-1">
            {[
              ...
              <React.Fragment key={to}>
                <RouterLink
                  to={to}
                  className="text-blue-300 hover:text-white text-xs transition-colors no-underline"
                >
                  {label}
                </RouterLink>
                {i < arr.length - 1 && (
