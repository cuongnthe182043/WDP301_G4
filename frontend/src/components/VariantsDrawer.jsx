    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-[480px] max-w-full bg-white shadow-xl z-50 overflow-y-auto p-4 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-bold">{t("shop.variant_management")}</h3>
          <button
            onClick={onClose}
        ...
        <div className="space-y-2">
          ...
          <Input size="sm" label={t("shop.price_col")} value={form.price} onValueChange={v => setForm({ ...form, price: v })} />
          <Input size="sm" label={t("shop.stock_short")} value={form.stock} onValueChange={v => setForm({ ...form, stock: v })} />
          <div className="flex gap-2">
            <Input
              size="sm"
              label={t("shop.size_label")}
              value={form.variant_attributes.size}
              onValueChange={v => setForm({ ...form, variant_attributes: { ...form.variant_attributes, size: v } })}
            />
            <Input
              size="sm"
              label={t("shop.color_label")}
              value={form.variant_attributes.color}
              onValueChange={v => setForm({ ...form, variant_attributes: { ...form.variant_attributes, color: v } })}
            />
          </div>
          <Button variant="bordered" size="sm" onPress={add}>{t("shop.add_one_variant")}</Button>
        </div>
        <Divider />
        ...
        <div className="space-y-2">
          {rows.map(v => (
            <div key={v._id} className="flex gap-2 items-center">
              ...
              <Input size="sm" label={t("shop.price_col")} value={String(v.price)} onValueChange={val => upd(v._id, { price: Number(val || 0) })} />
              <Input size="sm" label={t("shop.stock_short")} value={String(v.stock)} onValueChange={val => upd(v._id, { stock: Number(val || 0) })} />
              <button
