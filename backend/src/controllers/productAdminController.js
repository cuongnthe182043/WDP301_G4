...
const getShopId = (req) => req.shop?._id || req.user._id;

/* Products */
exports.listProducts = async (req, res, next) => {
    try {
        const data = await svc.listProducts({ shopId: getShopId(req), ...req.query });
        res.json({ success: true, data });
    } catch (e) { next(e); }
};
exports.getProduct = async (req, res, next) => {
    try {
        const doc = await svc.getProduct(req.params.id, getShopId(req));
        if (!doc) return res.status(404).json({ message: "Not found" });
        res.json({ success: true, data: doc });
    } catch (e) { next(e); }
};
exports.createProduct = async (req, res, next) => {
    try { res.json({ success: true, data: await svc.createProduct(req.body, getShopId(req)) }); }
    catch (e) { next(e); }
};
exports.updateProduct = async (req, res, next) => {
    try { res.json({ success: true, data: await svc.updateProduct(req.params.id, req.body, getShopId(req)) }); }
    catch (e) { next(e); }
};
exports.deleteProduct = async (req, res, next) => {
    try { res.json({ success: true, data: await svc.deleteProduct(req.params.id, getShopId(req)) }); }
    catch (e) { next(e); }
};

/* Variants */
exports.listVariants = async (req, res, next) => {
    try { res.json({ success: true, data: await svc.listVariants(req.params.id, getShopId(req)) }); }
    catch (e) { next(e); }
};
exports.createVariant = async (req, res, next) => {
    try { res.json({ success: true, data: await svc.createVariant(req.params.id, req.body, getShopId(req)) }); }
    catch (e) { next(e); }
};
exports.updateVariant = async (req, res, next) => {
    try { res.json({ success: true, data: await svc.updateVariant(req.params.variantId, req.body, getShopId(req)) }); }
    catch (e) { next(e); }
};
exports.deleteVariant = async (req, res, next) => {
    try { res.json({ success: true, data: await svc.deleteVariant(req.params.variantId, getShopId(req)) }); }
    catch (e) { next(e); }
};
...
exports.uploadImages = async (req, res, next) => {
    try { res.json({ success: true, data: await svc.uploadImages(req.files || [], getShopId(req)) }); }
    catch (e) { next(e); }
};
...
exports.importExcel = async (req, res, next) => {
    try {
        ...
        res.json({ success: true, data: await svc.importExcel(req.file.buffer, getShopId(req)) });
    }
    catch (e) { next(e); }
};
...
exports.lowStock = async (req, res, next) => {
    try {
        ...
        res.json({ success: true, data: await svc.lowStock(getShopId(req), threshold) });
    }
    catch (e) { next(e); }
};
...
exports.createVariantsBulk = async (req, res, next) => {
    try {
        const { rows } = req.body;
        res.json({ success: true, data: await svc.createVariantsBulk(req.params.id, rows, getShopId(req)) });
    }
    catch (e) { next(e); }
};
